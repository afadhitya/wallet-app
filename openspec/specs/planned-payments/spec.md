...
The system SHALL provide `wallet bill pay <id>` to manually fulfill a planned payment by creating a linked expense transaction and updating the planned payment state.
...
- **AND** marks the transaction as planned and links it to planned payment `1`
...
- **THEN** the system creates a linked expense transaction
...
- **THEN** the system creates the linked transaction with the overridden transaction date and amount
...
The system SHALL support JSON output for planned-payment commands whose contract includes `--json`.

#### Scenario: Render planned payment JSON
- **WHEN** the user runs a planned-payment command with `--json`
- **THEN** the system writes a machine-readable JSON response containing the command result fields
