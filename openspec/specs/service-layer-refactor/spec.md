...
The `internal/service/` directory is organized into domain sub-packages for maintainability: each core domain (transaction, planned payment, budget, report) lives in its own sub-package with files split by concern. A shared sub-package holds cross-cutting utilities. The `Service` struct embeds domain managers to preserve the public API surface.
